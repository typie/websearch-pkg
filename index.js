const {AbstractTypiePackage, TypieRowItem, AppGlobal} = require('typie-sdk');
...
class WebSearch extends AbstractTypiePackage
{
    ...
    populate(){
        ...
        Object.keys(this.pkgConfig.sites).map((siteName, i) => {
            ...
            this.packages[siteName] = new WebSearchPkgFactory(
                this.win, this.pkgConfig.sites[siteName], siteName, {
                    pkgName: siteName,
                    db: 'WebSearch',
                });
        });
    ...
    getIcon(siteName) {
        return this.pkgConfig.sites[siteName].icon ?  Path.join(this.iconPath, this.pkgConfig.sites[siteName].icon) : defaultUrlIco
    }
}
